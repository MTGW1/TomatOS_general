...
async def handle_message(self, message: Dict[str, Any]): # 中译中(bushi)
    """处理消息"""
    # 原始输入Webcli_messageItem对象
    # 最终输出Messagebase对象

    msg_item = Webcli_messageItem(**message)
    ...
    logger.info(f"收到Web终端消息: {msg_base}")

    return msg_base

async def send_message(msg: Messagebase):
    """发送消息"""
    # 转为发送对象
